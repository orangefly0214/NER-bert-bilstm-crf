...
if os.name == 'nt':
    ...
    bert_path = '/home/abc/PycharmProjects/bert/chinese_L-12_H-768_A-12/'
    root_path = '/home/abc/PycharmProjects/bert/MyNer'
    ...
    def _read_data(cls, input_file):
        ...
        with codecs.open(input_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                else:#每处理完一个句子中的所有单词及其标签，将这些单词重新合并为句子（当出现换行符的时候，一个句子结束，进行这个处理）
                    if len(contends) == 0:
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return bio_label(LABELS)
    # 验证数据的格式　guid,label,text
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    for (i, label) in enumerate(label_list, 1):
        label_map[label] = i
    # 保存label->index 的map　将标签和对应的id保存到label2id中
    ...
    # append("O") or append("[CLS]") not sure! 将单个样本对应的Label的4种格式并将它们机构化到feature这个类中
    ...
    input_ids = tokenizer.convert_tokens_to_ids(ntokens)  # 将序列中的字(ntokens)转化为词汇表中的ID形式
    ...
    while len(input_ids) < max_seq_length:
        ...
        ntokens.append("**NULL**")
        # label_mask.append(0)
    # print(len(input_ids))  句子长度都保持为128
...
def filed_based_convert_examples_to_features(
    ...
    # 遍历训练数据，将所有的训练数据转换为tfrecord的格式
    for (ex_index, example) in enumerate(examples):
        if ex_index % 5000 == 0:
            tf.logging.info("Writing example %d of %d" % (ex_index, len(examples)))
        # 对于每一个训练样本,将样本出处理为input_ids,input_mask,label_ids,segement_id的形式
        feature = convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode)

        def create_int_feature(values):
            f = tf.train.Feature(int64_list=tf.train.Int64List(value=list(values)))
            return f
        # 将单个样本的feature相关信息存储到features字典当中，并以序列化的方式写入文件，每得到一个样本写入一次
    ...
    def model_fn(features, labels, mode, params):
        ...
        for var in tvars:
            ...
            tf.logging.info("  name = %s, shape = %s%s", var.name, var.shape,
                            init_string)
        output_spec = None
        if mode == tf.estimator.ModeKeys.TRAIN: #训练阶段
...
#         raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    bert_config = modeling.BertConfig.from_json_file(FLAGS.bert_config_file)#bert的参数
    ...
    list_file=os.path.join(FLAGS.output_dir, 'label_list.pkl')
    print(list_file)
    if not os.path.exists(list_file):
        with open(list_file, mode='wb') as fd:
    ...
    if FLAGS.do_eval:
        if data_config.get('eval.tf_record_path', '') == '':
            ...
            filed_based_convert_examples_to_features(
                eval_examples, label_list, FLAGS.max_seq_length, tokenizer, eval_file) #将样本转换为tf_record的形式
        ...
        tf.logging.info("  Num examples = %d", num_eval_size) #验证集中的样本条数
        ...
        #得到验证集上的结果，并将验证集的结果写入eval.txt 通过estimator接口对数据进行进行预测
        result = estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps)
    ...
    if FLAGS.do_predict:
        ...
        #获取测试数据
        predict_examples = processor.get_test_examples(FLAGS.data_dir) #获得封装好的测试数据
        ...
        print("测试集上的包含sentences:{}".format(len(predict_examples)))
        ...
        #将测试集上的数据结果写入label_test.txt
        output_predict_file = os.path.join(FLAGS.output_dir, "label_test.txt")

        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result): #从result中解析出prediction 的结果
                idx = 0
                line = ''
                ...
                for id in prediction:
                    ...
                    idx += 1
                writer.write(line + '\n')
        # 将模型预测的结果和原始标签写入到文件中，以空格分开，使用conevel.py脚本来预测entity level 的结果并且输出 label.txt中存的是最终的原结果＋测试结果
