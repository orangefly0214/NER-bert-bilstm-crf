...
def predict_online():
    ...
    :return:
    """
    def convert(line): #讲数据转换为[batch_size,max_seq_len]的形式
    ...
    with graph.as_default():
        ...
        while True:
            ...
            pred_ids_result = sess.run([pred_ids], feed_dict) #预测出来的句子标签的id值
            pred_label_result = convert_id_to_label(pred_ids_result, id2label) #将标签的id转换为对应的标签名称
...
def strage_combined_link_org_loc(tokens, tags):
    ...
    labels,others= eval.get_result(tokens, tags)
    for key in labels:
        print_output(labels[key],key)
...
class Result(object):
    def __init__(self, config):
        self.config = config
        self.labels = {
            "Investor": [],"Investee":[],"Transferor":[],"Transferee":[],
            "RC(B)":[],"RC(A)":[],"ACC(CI)":[],"ACC(CD)":[],"ACC(ST)":[],
            "Change of RC(CI)":[],"Change of RC(CD)":[],"Change of RC(ST)":[],
            "Change of CR(CI)":[],"Change of CR(CD)":[], "Change of CR(ST)":[],
            "Valuation(B)":[], "Valuation(A)":[],"Ratio(B)":[],"Ratio(A)":[], "Change of ratio(CI)":[],
            "Change of ratio(CD)":[],"Change of ratio(ST)":[]
        }
        self.others=[]
    ...
    def get_result(self, tokens, tags, config=None):
        ...
        return self.labels,self.others
